    ...
    info['pairs'].each do |pair|
      ...
      if (base_found.present? && quote_found.present?)
        ...
        Pair.create! base_id: base_found.id, quote_id: quote_found.id, name: pair_name, code: code,
                     ...
                     fee:        values['fee'],        status:    'archived'
      end
    end
  end
...
#if Run.count == 0
#  Run.create pair_id: 1, kind: "ask", depo: 0.1e4, last: 0.17061367e4, indent: 5.0, overlap: 10.0, martingale: 15.0, orders_number: 6, profit: 2.0, scale: "logarithmic", stop_loss: 0.8616e4
#end
#
#if FixOrder.count == 0
#  run    = Run.first
